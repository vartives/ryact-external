...
#include "../../imgui/examples/example_win32_directx11/custom_widgets.h"
...
inline void ChangeKeyFreeze(void* blank)
{
...
static void HotkeyButton(int aimkey, void* changekey, int status)
{
	...
	/*if (custom::Button(aimkeys.c_str(), ImVec2(125, 20)))
	{
		if (status == 0)
		{
...
static void HotkeyButton2(int aimkey, void* changekey, int status)
{
	const char* preview_value = "Press Key";
	if (aimkey >= 0 && aimkey < IM_ARRAYSIZE(keyNames))
		Items_ArrayGetter(keyNames, aimkey, &preview_value);
